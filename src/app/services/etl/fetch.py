...
def fetch_fulltext_pmc(pmcid: str) -> Optional[str]:
    ...
    try:
        ...
        for noise in body.find_all(["ref-list", "fig", "re-list"]):
            noise.decompose()
        ...
        for table_wrap in body.find_all("table-wrap"):
            ...
            formatted_table = "\n" + "\n".join(table_text) + "\n"
            table_wrap.replace_with(formatted_table)
...
def get_db_connection():
    conn = psycopg2.connect(
        dbname="tfg_pipeline_validation", # tfg_pipeline
        user="",
        password="",
        host="localhost"
    )
...
def store_in_pgvector(title, text, source_url, conn):
    ...
    data_to_insert = [
        (title, source_url, chunks[i], embeddings[i])
...
def get_project_root():
    ...
    while current_path != os.path.dirname(current_path):
        if "src" in os.listdir(current_path) and os.path.isdir(os.path.join(current_path, "data")):
            return current_path
...
def ingest_all():
    ...
    try:
        ...
        metadata_path = os.path.join(root, "data", "metadata_validation.json") # metadata_index.json
